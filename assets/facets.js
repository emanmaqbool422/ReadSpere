...
if (!customElements.get('custom-select')) {
  class CustomSelect extends HTMLElement {
    ...
    connectedCallback() {
      this.button = this.querySelector('.custom-select__button');
      this.list = this.querySelector('.custom-select__list');
      this.options = this.querySelectorAll('.custom-select__option');
      this.select = this.querySelector('select');
      this.selectedText = this.querySelector('.custom-select__selected-text');

      if (!this.button || !this.list || !this.select) return;
    ...
    toggle() {
      if (this.isOpen()) {
        ...
        document.querySelectorAll('custom-select').forEach((cs) => {
          if (cs !== this && typeof cs.close === 'function') cs.close();
        });
        this.open();
      }
    }
    ...
    selectOption(value, text) {
      if (this.select.value !== value) {
        ...
        this.options.forEach((opt) => {
          const isSel = opt.getAttribute('data-value') === value;
          opt.classList.toggle('is-selected', isSel);
          opt.setAttribute('aria-selected', isSel ? 'true' : 'false');
          let checkIcon = opt.querySelector('.custom-select__check');
          if (isSel) {
            if (!checkIcon) {
              opt.insertAdjacentHTML('beforeend', `<svg class="custom-select__check" width="14" height="10" viewBox="0 0 14 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 5L5 9L13 1" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`);
            }
          } else {
